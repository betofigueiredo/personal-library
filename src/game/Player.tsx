...
export function Player() {
  ...
  const speed = useRef(0); // horizontal ground speed, units/s
  ...
  useFrame((state, delta) => {
    ...
    // Actual ground speed the cat is commanded to move at — drives the leg
    // gait cadence so the paws keep pace and don't slide.
    speed.current = Math.hypot(dir.x, dir.z);
    ...
    for (let i = 0; i < SECTION_SPOTS.length; i++) {
      ...
      if (!section) continue;
      const r =
        game.activeSectionId === section.id ? COURT_EXIT_R : COURT_ENTER_R;
